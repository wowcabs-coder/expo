...
async function transformJSWithBabel(
  ...
  const jsFile: JSFile = {
    ...
    functionMap:
      ...
      null,
    // @ts-expect-error: `hasCjsExports` is set through babel-preset-expo/src/detect-dynamic-exports.ts
    hasCjsExports: transformResult.metadata?.hasCjsExports,
    // @ts-expect-error: `reactClientReference` is set through packages/babel-preset-expo/src/client-module-proxy-plugin.ts
    reactClientReference: transformResult.metadata?.reactClientReference,
    // @ts-expect-error: `expoDomComponentReference` is set through packages/babel-preset-expo/src/use-dom-directive-plugin.ts
